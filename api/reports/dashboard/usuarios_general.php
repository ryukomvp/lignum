...
if ($dataUsuarios = $usuario->readAll()) {
    ...
    foreach ($dataUsuarios as $rowUsuario) {
        ...
        if ($usuario->setAcceso($rowUsuario['acceso'])) {
            // Se verifica si existen registros para mostrar, de lo contrario se imprime un mensaje.
            if ($dataProductos = $usuario->productosCategoria()) {
                // Se recorren los registros fila por fila.
                foreach ($dataUsuarios as $rowUsuario) {
                    ($rowUsuario['acceso']) ? $acceso = 'Permitido' : $acceso = 'Denegado';
                    // Se imprimen las celdas con los datos de los productos.
                    $pdf->cell(46.5, 10, $pdf->encodeString($rowUsuario['"Nombre completo"']), 1, 0);
                    $pdf->cell(46.5, 10, $rowUsuario['precio_producto'], 1, 0);
                    $pdf->cell(46.5, 10, $pdf->encodeString($rowUsuario['tipo_material']), 1, 0);
                }
            } else {
